    ...
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-gray-50/50 font-sans text-foreground">
        ...
        <ModalFrame
          ...
          header={
            <div className="flex items-center justify-between border-b border-border bg-background px-6 py-5">
              ...
              <button onClick={() => setIsCreateModalOpen(false)} className="text-foreground/50 hover:text-foreground transition-colors p-2">X</button>
            </div>
          }
        >
      ...
      {confirmingCreate ? (
        <ConfirmDialog
          ...
          description={
            <span>
              Você está prestes a provisionar a instância <strong>{formData.nome}</strong> vinculada à unidade de negócio <strong>{formData.unidade_negocio_id}</strong>. Deseja prosseguir?
            </span>
          }
